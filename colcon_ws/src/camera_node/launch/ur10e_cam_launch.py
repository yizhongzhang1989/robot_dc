#!/usr/bin/env python3
"""
UR10e Camera Launch File

Starts a ``camera_node`` instance configured from ``ur10e.camera`` in
robot_config.yaml. Uses a distinct node name (``ur10e_camera_node``) and
disjoint server port + ROS topic so it can run alongside the ur15 camera and
(optionally) the duco camera.
"""
...
def generate_launch_description():
    ...
    ur10e = config.get_robot('ur10e')
    camera_config = ur10e.get('camera')
    ...
    # Distinct node name so this instance can coexist with the ur15 camera node.
    camera_node = Node(
        package='camera_node',
        executable='camera_node',
        name='ur10e_camera_node',
        parameters=[
