...
def test_standalone_functions(node):
    ...
    from robot_status_redis.client_utils import get_from_status, set_to_status
    ...
    success = set_to_status(namespace, 'temperature', 23.5)
    assert success
    
    success = set_to_status(namespace, 'sensor_config', {
        'id': 'sensor_001',
    ...
    success = set_to_status(namespace, 'recent_readings', readings)
    assert success
    
    # Test getting values back
    temp = get_from_status(namespace, 'temperature')
    node.get_logger().info(f"Temperature: {temp}°C")
    assert temp == 23.5
    
    config = get_from_status(namespace, 'sensor_config')
    node.get_logger().info(f"Sensor config: {config['id']} at {config['location']}")
    assert config['id'] == 'sensor_001'
    
    readings_retrieved = get_from_status(namespace, 'recent_readings')
    ...
    missing = get_from_status(namespace, 'nonexistent_key')
