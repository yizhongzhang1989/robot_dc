...
ros2 run ur_workflow run_workflow.py --config /path/to/my_workflow.yaml

# Dry run (validate only)
ros2 run ur_workflow run_workflow.py --config workflow.yaml --dry-run
```
...
run_workflow --config $(ros2 pkg prefix ur_workflow)/share/ur_workflow/examples/workflow_simple.yaml
```
...
```python
import rclpy
from ur_workflow.engine import WorkflowEngine
from ur_workflow.handlers import register_all_handlers
...
Installed location: `$(ros2 pkg prefix ur_workflow)/share/ur_workflow/examples/`
...
4. Run using `ros2 run ur_workflow run_workflow.py --config your_workflow.yaml`

## Extending with Custom Handlers

1. Create a new handler in `ur_workflow/handlers/`
2. Register it in `ur_workflow/handlers/__init__.py`
3. Rebuild the package: `colcon build --packages-select ur_workflow`
...
```bash
ros2 run ur_workflow run_workflow.py --config your_workflow.yaml --dry-run
```
