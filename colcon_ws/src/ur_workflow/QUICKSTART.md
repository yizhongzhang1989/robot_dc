...
ros2 run ur_workflow run_workflow.py \
  --config $(ros2 pkg prefix ur_workflow)/share/ur_workflow/examples/workflow_example.yaml
...
$(ros2 pkg prefix ur_workflow)/share/ur_workflow/examples/
...
```bash
cp $(ros2 pkg prefix ur_workflow)/share/ur_workflow/examples/workflow_simple.yaml my_workflow.yaml
```
...
3. Run it:
```bash
ros2 run ur_workflow run_workflow.py --config my_workflow.yaml
```

## Adding Custom Operation Handlers

1. Create new handler in `ur_workflow/handlers/my_handler.py`
2. Import and register in `ur_workflow/handlers/__init__.py`
3. Rebuild:
```bash
cd ~/Documents/robot_dc/colcon_ws
colcon build --packages-select ur_workflow
...
colcon build --packages-select ur_workflow other_package
```
