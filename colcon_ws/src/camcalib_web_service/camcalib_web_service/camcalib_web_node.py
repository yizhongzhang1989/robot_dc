#!/usr/bin/env python3
"""
Camera Calibration Web Service ROS2 Node

This node wraps the camera calibration toolkit web service and manages its lifecycle.
It launches the Flask web application for camera calibration.
"""
...
class CamCalibWebServiceNode(Node):
    """ROS2 node that manages the camera calibration web service."""
    
    def __init__(self):
        super().__init__('camcalib_web_service_node')
        ...
        # Log the found workspace root for debugging
        self.get_logger().info(f"Found workspace root: {workspace_root}")
        
        # Store workspace root as Path object
        self.workspace_root = Path(workspace_root)
        
        self.app_path = self.workspace_root / 'scripts' / 'ThirdParty' / 'camera_calibration_toolkit' / 'web' / 'app.py'
        
        if not self.app_path.exists():
            self.get_logger().error(f"app.py not found at: {self.app_path}")
            self.get_logger().error("Please ensure the camera_calibration_toolkit exists at scripts/ThirdParty/")
            sys.exit(1)
        ...
        self.get_logger().info("Camera Calibration Web Service Configuration:")
    ...
    def start_service(self):
        """Start the camera calibration web service."""
        try:
            # Build command - Use Flask CLI to override the hardcoded port
            cmd = [
                'flask',
            ...
            env = os.environ.copy()
            env['FLASK_APP'] = str(self.app_path)
            
            self.get_logger().info(f"Starting service: {' '.join(cmd)}")
            self.get_logger().info(f"Environment: FLASK_RUN_HOST={self.host}, FLASK_RUN_PORT={self.port}")
            ...
            for line in iter(self.process.stdout.readline, ''):
                if line:
                    ...
                    if 'ERROR' in line or 'Error' in line or 'error' in line:
                        self.get_logger().error(f"[camcalib_web] {line}")
                    elif 'WARNING' in line or 'Warning' in line or 'warning' in line:
                        self.get_logger().warn(f"[camcalib_web] {line}")
                        ...
                        continue
                    elif 'Running on' in line or 'Press CTRL+C' in line or '=' * 10 in line:
                        # Log startup messages
                        self.get_logger().info(f"[camcalib_web] {line}")
                    elif line.strip():
                        # Log other non-empty lines at debug level
                        self.get_logger().debug(f"[camcalib_web] {line}")
        except Exception as e:
    ...
    def cleanup(self):
        ...
        if self.process and self.process.poll() is None:
            try:
                self.get_logger().info("Stopping camcalib_web service...")
                ...
                try:
                    ...
                    self.get_logger().info("Service process already terminated")
                    return
                except Exception as e:
                    self.get_logger().warn(f"Error sending SIGTERM: {e}")
            ...
            self.get_logger().info("Service process already exited")
...
def main(args=None):
    ...
    try:
        # Create node
        node = CamCalibWebServiceNode()
        
        # Keep the node alive - spin will handle SIGINT internally
        rclpy.spin(node)
        
    except KeyboardInterrupt:
        # This is caught when Ctrl+C is pressed
        if node:
        ...
        # Always cleanup - this ensures proper shutdown
        if node:
            try:
                node.cleanup()
            except Exception as e:
