...
try:
    ...
    ur15_pkg_path = os.path.join(
        os.path.dirname(get_package_share_directory('ur_web')),
        'ur_robot_arm'
    )
...
class UR15WebNode(Node):
    ...
    def setup_flask_routes(self):
        ...
        def run_workflow():
            """Run a workflow using ros2 run ur_workflow run_workflow.py."""
            from flask import request, jsonify
            from common.workspace_utils import get_workspace_root
            import subprocess
            import threading
            ...
            try:
                ...
                command = ['ros2', 'run', 'ur_workflow', 'run_workflow.py', '--config', workflow_path]
