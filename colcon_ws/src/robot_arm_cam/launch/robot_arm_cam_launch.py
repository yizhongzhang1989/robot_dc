from launch import LaunchDescription
from launch_ros.actions import Node
from launch.actions import DeclareLaunchArgument
from launch.substitutions import LaunchConfiguration


def generate_launch_description():
    
    # Declare launch arguments
    camera_name_arg = DeclareLaunchArgument(
        'camera_name',
        default_value='RobotArmCamera',
        description='Name of the camera'
    )
    
    rtsp_url_1080p_arg = DeclareLaunchArgument(
        'rtsp_url_1080p',
        default_value='rtsp://admin:123456@192.168.1.102/stream0',
        description='RTSP URL for 1080p stream'
    )
    
    rtsp_url_360p_arg = DeclareLaunchArgument(
        'rtsp_url_360p',
        default_value='rtsp://admin:123456@192.168.1.102/stream1',
        description='RTSP URL for 360p stream'
    )
    
    camera_ip_arg = DeclareLaunchArgument(
        'camera_ip',
        default_value='192.168.1.102',
        description='IP address of the camera'
    )
    
    server_port_arg = DeclareLaunchArgument(
        'server_port',
        default_value='8011',
        description='Port for Flask web server'
    )
    
    stream_fps_arg = DeclareLaunchArgument(
        'stream_fps',
        default_value='25',
        description='Target FPS for video streaming'
    )
    
    jpeg_quality_arg = DeclareLaunchArgument(
        'jpeg_quality',
        default_value='75',
        description='JPEG compression quality (1-100)'
    )
    
    max_width_arg = DeclareLaunchArgument(
        'max_width',
        default_value='800',
        description='Maximum width for video streaming'
    )
    
    ros_publish_rate_arg = DeclareLaunchArgument(
        'ros_publish_rate',
        default_value='10.0',
        description='ROS2 image publishing rate (Hz)'
    )
    
    # Robot arm camera node
    robot_arm_cam_node = Node(
        package='robot_arm_cam',
        executable='robot_arm_cam',
        name='robot_arm_cam',
        output='screen',
        parameters=[{
            'camera_name': LaunchConfiguration('camera_name'),
            'rtsp_url_1080p': LaunchConfiguration('rtsp_url_1080p'),
            'rtsp_url_360p': LaunchConfiguration('rtsp_url_360p'),
            'camera_ip': LaunchConfiguration('camera_ip'),
            'server_port': LaunchConfiguration('server_port'),
            'stream_fps': LaunchConfiguration('stream_fps'),
            'jpeg_quality': LaunchConfiguration('jpeg_quality'),
            'max_width': LaunchConfiguration('max_width'),
            'ros_publish_rate': LaunchConfiguration('ros_publish_rate'),
        }]
    )
    
    return LaunchDescription([
        camera_name_arg,
        rtsp_url_1080p_arg,
        rtsp_url_360p_arg,
        camera_ip_arg,
        server_port_arg,
        stream_fps_arg,
        jpeg_quality_arg,
        max_width_arg,
        ros_publish_rate_arg,
        robot_arm_cam_node,
    ])
