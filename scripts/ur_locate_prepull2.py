#!/usr/bin/env python3
"""
UR Locate PrePull2 Script
This script inherits from URLocateBase and customizes it for prepull2 location tasks.
"""
...
class URLocatePrePull2(URLocateBase):
    def __init__(self, api_url="http://10.172.100.34:8001", robot_ip="192.168.1.15", robot_port=30002):
        ...
        self.data_dir = os.path.join(self.script_dir, '..', 'temp', 'ur_locate_prepull2_data')
        
        # Override result directory path
        self.result_dir = os.path.join(self.script_dir, '..', 'temp', 'ur_locate_prepull2_result')
        ...
        # For prepull2 location, use keypoint 0 to keypoint 1 to define X-axis
        ...
        print(f"URLocatePrePull2 initialized with custom settings:")
...
def main():
    """
    Main function for URLocatePrePull2
    """
    # Initialize ROS2
    rclpy.init()
    
    try:
        # Initialize URLocatePrePull2 instance (robot connection is handled internally)
        ur_prepull2 = URLocatePrePull2()
        
        # Check if robot was initialized successfully
        if ur_prepull2.robot is None or not ur_prepull2.robot.connected:
            print("✗ Robot initialization failed. Please check robot connection and try again.")
            return
        
        # Use multi-threaded executor to handle callbacks
        executor = MultiThreadedExecutor()
        executor.add_node(ur_prepull2)
        ...
        try:
            ...
            if ur_prepull2.auto_collect_data():
                ...
                if ur_prepull2.estimate_3d_position():
                    ...
                    if coord_system:
                        ...
                        print("="*60)
                        if ur_prepull2.validate_local_coordinate_system(coord_system):
                            ...
                            # print("="*60)
                            # angle_result = ur_prepull2.calculate_kp3_kp4_rotation_angle()
                            # if angle_result:
            ...
            if ur_prepull2.robot is not None:
                try:
                    ur_prepull2.robot.close()
