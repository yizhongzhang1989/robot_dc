...
# ===========================functions for FTC move demonstration===========================
'''FTC Task for demonstration: linear move'''
...
'''FTC Task for demonstration: rotation around an axis'''
def FTC_task_rotatez(ftSet, stop_type, params):
...
def main():
    ...
    # # Note that the direction of force 
    # ftSet = [0,0,-20,0,0,0] # move to z+ in tool frame
    # stop_type = 'distance'
    # params = 50   # mm
    # FTC_task_moveline(ftSet, stop_type, params)

    # Note that for rotation, the end condition angle must be positive value, the direction of the motion is defined by torque 
    ftSet = [0,0,0,0,0,5] # counter-clock with z axis
    stop_type = 'angle'
    params = [0,0,0,0,0,90]   # deg, the angle here is not equal to the real angle
    FTC_task_rotatez(ftSet, stop_type, params)
