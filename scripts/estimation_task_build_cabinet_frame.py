...
Input:
- 3D positioning results from 3d_coordinates_estimation_result.json

Output:
- New coordinate system transformation matrix
- Cabinet pose information (position and orientation)
- Results saved to: temp/build_cabinet_frame_result/build_cabinet_frame_result.json
"""
...
class CabinetFrameBuilder:
    ...
    def build_coordinate_system(self, point2_id: int = 2, point8_id: int = 8) -> np.ndarray:
        ...
        logger.info("Ideal cabinet X+ = Base Y+, reference value =  [0.00000000, 1.00000000, 0.00000000]")

        
        # Use base coordinate system Z-axis
        z_axis = np.array([0.0, 0.0, 1.0])
        logger.info(f"Z-axis direction (same as base Z-axis): {z_axis}")
        logger.info("Ideal cabinet Z+ = Base Z+, reference value =  [0.00000000, 0.00000000, 1.00000000]")
        ...
        logger.info("Ideal cabinet Y+ = Base X-, reference value =  [-1.00000000, 0.00000000, 0.00000000]")
    ...
    def save_results(self, results: Dict, output_file: str = "build_cabinet_frame_result.json"):
        """
...
def main():
    ...
    try:
        ...
        print(f"   Results saved to: {builder.output_dir / 'build_cabinet_frame_result.json'}")
