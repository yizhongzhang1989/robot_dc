...
class URExecuteClose(URExecuteBase):
    ...
    def movel_to_right_handle_close_position(self):
        """
        Move robot to right close position using linear movement (movel) in two steps:
        1. Move along Y and Z directions (with -0.21m Y offset for right close)
        2. Move along X direction
        Returns: 0 if successful, error code otherwise
        """
        if self.robot is None:
        ...
        # Apply -0.28m offset in Y direction for right close
        right_close_y = target_position[1] - 0.26
        
        # Step 1: Move along Y and Z directions (keep X unchanged)
        intermediate_pose = [
            current_pose[0],      # x (keep current X)
            right_close_y,         # y (move to target Y with -0.21m offset)
            target_position[2]+0.01,   # z (move to target Z)
            current_pose[3],      # rx (keep current orientation)
            current_pose[4],      # ry
            current_pose[5]       # rz
        ]
        
        print("\n[Right Close] Step 1: Moving along Y and Z directions...")
        print(f"Intermediate pose: {intermediate_pose}")
        ...
        # Step 2: Move along X direction to final target
        final_pose = [
            target_position[0]-0.2,  # x (move to target X with tool offset)
            right_close_y,             # y (already at right close Y position)
            target_position[2]+0.01,       # z (already at target Z)
            current_pose[3],          # rx (keep current orientation)
            current_pose[4],          # ry
            current_pose[5]           # rz
        ]
        
        print("\n[Right Close] Step 2: Moving along X direction...")
        print(f"Final pose: {final_pose}")
    ...
    def movel_to_leave_handle(self, distance):
        """
        Move the robot away from the handle by moving "distance" meter in the negative X direction.
        Returns: 0 if successful, error code otherwise
        """
        if self.robot is None:
            print("Robot is not initialized")
        ...
        print("\n[INFO] Moving away from close (X- direction, 0.2m)...")
        ...
        if res == 0:
            print("[INFO] Successfully moved away from close")
        else:
            print(f"[ERROR] Failed to move away from close (error code: {res})")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    ur_close.movel_to_leave_handle(-0.2)
    time.sleep(0.5)
    ...
    # # move to leave the handle
    print("\n" + "="*50)
    ur_close.movel_to_leave_handle(-0.3)
    time.sleep(0.5)
