#!/usr/bin/env python3
"""
UR Locate PrePull Script
This script inherits from URLocateBase and customizes it for prepull location tasks.
"""
...
class URLocatePrePull(URLocateBase):
    def __init__(self, api_url="http://10.172.100.34:8001", robot_ip="192.168.1.15", robot_port=30002):
        """
        Initialize URLocatePrePull class for UR robot prepull location tasks
        ...
        self.result_dir = os.path.join(self.script_dir, '..', 'temp', 'ur_locate_prepull_result')
        ...
        # For prepull location, use keypoint 0 to keypoint 1 to define X-axis
        ...
        print(f"URLocatePrePull initialized with custom settings:")
...
def main():
    """
    Main function for URLocatePrePull
    """
    # Initialize ROS2
    rclpy.init()
    
    try:
        # Initialize URLocatePrePull instance (robot connection is handled internally)
        ur_prepull = URLocatePrePull()
        
        # Check if robot was initialized successfully
        if ur_prepull.robot is None or not ur_prepull.robot.connected:
            print("✗ Robot initialization failed. Please check robot connection and try again.")
            return
        
        # Use multi-threaded executor to handle callbacks
        executor = MultiThreadedExecutor()
        executor.add_node(ur_prepull)
        ...
        try:
            ...
            if ur_prepull.auto_collect_data():
                ...
                if ur_prepull.estimate_3d_position():
                    ...
                    if coord_system:
                        ...
                        print("="*60)
                        if ur_prepull.validate_local_coordinate_system(coord_system):
                            ...
                            # print("="*60)
                            # angle_result = ur_prepull.calculate_kp3_kp4_rotation_angle()
                            # if angle_result:
            ...
            if ur_prepull.robot is not None:
                try:
                    ur_prepull.robot.close()
