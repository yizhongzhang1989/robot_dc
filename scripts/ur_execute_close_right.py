...
class URExecuteCloseRight(URExecuteBase):
...
# ================================= Movement Functions ================================
    ...
    def movel_to_target_position(self):
        ...
        final_movement_y = (movement_crack_local_y - 0.40) * crack_local_y
...
# ================================ Force Control Functions ================================
    def force_task_touch_right_handle(self):
        ...
        result = self.robot.force_control_task(
            ...
            damping=0.05,
            end_type=3,
            end_distance=[0.02,0.02,0.09,0,0,0]
        )
        time.sleep(0.5)
        return result
    ...
    def force_task_close_right_handle(self):
        """
        Execute force control task to unlock the left close_right using TCP coordinate system.
        """
        if self.robot is None:
            print("Robot is not initialized")
            return -1
        ...
        # ==============Motion 1: push to close_right the handle=================
        print("\n[Motion 1] Pushing to close_right the handle...")
        ...
        result1 = self.robot.force_control_task(
            ...
            damping=0.05,
            end_type=3,
            end_distance=[0.15,0.05,0.10,0,0,0]
        )
        ...
        # ==============Motion 3: push handle to completely close_right=================
        print("\n[Motion 3] Pushing handle to completely close_right...")
        ...
        result1 = self.robot.force_control_task(
            ...
            damping=0.05,
            end_type=3,
            end_distance=[0.07,0.05,0.20,0,0,0]
        )
        
        if result1 != 0:
            print(f"[ERROR] Motion 6 failed with code: {result1}")
            return result1
    ...
    def force_task_prepull_server(self):
        ...
        result = self.robot.force_control_task(
            ...
            damping=0.1,
            end_type=3,
            end_distance=[0,0,0.15,0,0,0]
        )
        time.sleep(0.5)
        return result

if __name__ == "__main__":
    # Create URExecuteCloseRight instance
    ur_close_right = URExecuteCloseRight()
    ...
    if ur_close_right.estimated_keypoints is not None:
        print("\nEstimated Keypoints:")
        for kp in ur_close_right.estimated_keypoints:
    ...
    if ur_close_right.local_transformation_matrix is not None:
        ...
        print(ur_close_right.local_transformation_matrix)
    ...
    if ur_close_right.task_position_offset is not None:
        print("\nTask Position Offset (in local coordinate system):")
        print(f"x: {ur_close_right.task_position_offset.get('x', 0):.6f}")
        print(f"y: {ur_close_right.task_position_offset.get('y', 0):.6f}")
        print(f"z: {ur_close_right.task_position_offset.get('z', 0):.6f}")

    # Calculate target position in base coordinate system
    target_position = ur_close_right.get_target_position()
    ...
    print("\n" + "="*50)
    ur_close_right.movej_to_reference_joint_positions()
    time.sleep(0.5)
    
    # align tool TCP with local coordinate system
    print("\n" + "="*50)
    ur_close_right.movel_to_correct_tool_tcp()
    time.sleep(0.5)
    
    # move to target position using linear movement
    print("\n" + "="*50)
    ur_close_right.movel_to_target_position()
    time.sleep(0.5)

    # move to touch left handle
    print("\n" + "="*50)
    ur_close_right.force_task_touch_right_handle()
    time.sleep(0.5)

    # execute close_right left handle
    print("\n" + "="*50)
    ur_close_right.force_task_close_right_handle()
    time.sleep(0.5)

    # # move to leave the handle
    print("\n" + "="*50)
    ur_close_right.movel_in_crack_frame([0, -0.10, 0])
    time.sleep(0.5)
    ...
    # # move to the handle
    print("\n" + "="*50)
    ur_close_right.movel_in_crack_frame([-0.11, 0.095, 0])
    time.sleep(0.5)

    # execute force task to push server to the end
    print("\n" + "="*50)
    ur_close_right.force_task_touch_server()
    time.sleep(0.5)

    # execute force task to pull server out again
    print("\n" + "="*50)
    ur_close_right.force_task_prepull_server()
    time.sleep(0.5)
    ...
    # # move to leave the handle
    print("\n" + "="*50)
    ur_close_right.movel_in_crack_frame([0, -0.15, 0])
    time.sleep(0.5)
