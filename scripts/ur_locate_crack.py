...
class URLocateCrack(URLocateBase):
    """
    Custom location test class for crack task
    ...
    def __init__(self, api_url="http://10.172.100.34:8001", robot_ip="192.168.1.15", robot_port=30002):
        """
        Initialize URLocateCrack class
        ...
        print(f"✓ URLocateCrack initialized")
...
def main():
    """
    Main function for URLocateCrack task
    ...
    try:
        # Initialize URLocateCrack instance
        print("="*60)
        print("UR Locate Crack Task")
        print("="*60)
        
        ur_crack = URLocateCrack()
        
        # Check if robot was initialized successfully
        if ur_crack.robot is None or not ur_crack.robot.connected:
            print("✗ Robot initialization failed. Please check robot connection and try again.")
            return
        
        # Use multi-threaded executor to handle callbacks
        executor = MultiThreadedExecutor()
        executor.add_node(ur_crack)
        ...
        print("="*60)
        if not ur_crack.load_camera_parameters():
        ...
        try:
            ...
            if ur_crack.auto_collect_data():
                ...
                if ur_crack.estimate_3d_position():
                    ...
                    if coord_system:
                        ...
                        print("="*60)
                        if ur_crack.validate_local_coordinate_system(coord_system):
            ...
            if ur_crack.robot is not None:
                try:
                    ur_crack.robot.close()
            ...
            executor.shutdown()
            # Destroy ROS node
            ur_crack.destroy_node()
    
    finally:
        # Shutdown ROS2
        rclpy.shutdown()
        print("\n" + "="*60)
        print("UR Locate Crack Task Finished")
        print("="*60)
