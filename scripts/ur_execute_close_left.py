...
class URExecuteCloseLeft(URExecuteBase):
    def __init__(self, robot_ip="192.168.1.15", robot_port=30002):
        # Call parent class constructor first
        super().__init__(robot_ip, robot_port)
        
        # Override data_dir and result_dir with close_left-specific paths
        self.data_dir = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "temp",
            "ur_locate_close_left_data"
        )
        
        self.result_dir = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "temp",
            "ur_locate_close_left_result"
        )
...
# ================================ Force Control Functions ================================
    ...
    def force_task_close_left_handle(self):
        """
        Execute force control task to unlock the left close_left using TCP coordinate system.
        """
        if self.robot is None:
            print("Robot is not initialized")
            return -1
        ...
        # ==============Motion 0: push to close_left the handle=================
        print("\n[Motion 0] Pushing to close_left the handle...")
        ...
        # ==============Motion 1: push to close_left the handle=================
        print("\n[Motion 1] Pushing to close_left the handle...")
        ...
        result1 = self.robot.force_control_task(
            ...
            damping=0.05,
            end_type=3,
            end_distance=[0.12,0.05,0.10,0,0,0]
        )
        ...
        # ==============Motion 3: push handle to completely close_left=================
        print("\n[Motion 3] Pushing handle to completely close_left...")
...
if __name__ == "__main__":
    ...
    ur_close_left = URExecuteCloseLeft()
    ...
    if ur_close_left.estimated_keypoints is not None:
        print("\nEstimated Keypoints:")
        for kp in ur_close_left.estimated_keypoints:
    ...
    if ur_close_left.local_transformation_matrix is not None:
        ...
        print(ur_close_left.local_transformation_matrix)
    ...
    if ur_close_left.task_position_offset is not None:
        print("\nTask Position Offset (in local coordinate system):")
        print(f"x: {ur_close_left.task_position_offset.get('x', 0):.6f}")
        print(f"y: {ur_close_left.task_position_offset.get('y', 0):.6f}")
        print(f"z: {ur_close_left.task_position_offset.get('z', 0):.6f}")

    # Calculate target position in base coordinate system
    target_position = ur_close_left.get_target_position()
    ...
    print("\n" + "="*50)
    ur_close_left.movej_to_reference_joint_positions()
    time.sleep(0.5)
    
    # align tool TCP with local coordinate system
    print("\n" + "="*50)
    ur_close_left.movel_to_correct_tool_tcp()
    time.sleep(0.5)
    ...
    print("\n" + "="*50)
    ur_close_left.force_task_touch_left_handle()
    time.sleep(0.5)

    # execute close_left left handle
    print("\n" + "="*50)
    ur_close_left.force_task_close_left_handle()
    time.sleep(0.5)
