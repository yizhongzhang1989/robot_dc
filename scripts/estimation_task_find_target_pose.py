"""
Robot Target Pose Estimation Script

Inputs:
- temp/build_cabinet_frame_result/build_cabinet_frame_result.json: Cabinet-to-base transformation matrix
- temp/positioning_data/target_pose.json: Task pose in cabinet coordinate system

Outputs:
- temp/find_target_result/find_target_results.jpg: 3D visualization of coordinate systems
- Console output: Target joint angles, pose accuracy analysis

Functionality:
1. Transforms target pose from cabinet coordinates to robot base coordinates
2. Calculates required joint angles using inverse kinematics
3. Validates accuracy with forward kinematics
4. Visualizes coordinate systems and target positions
5. Provides detailed accuracy assessment report
"""
...
def calculate_target_joint_angles(robot):
    ...
    print("\nCalculating target joint angles based on cal_ikine...")
...
def visualize_coordinate_systems(robot=None, target_joint_angles=None):
    ...
    - Ideal target frame (from coordinate transformation)
    ...
    # Draw Ideal target frame (from coordinate transformation)
    draw_coordinate_frame(ax, task_pos_base, task_rot_base, 
                         'Ideal Target Frame', size=0.08, alpha=0.8)
    ...
    # Use fixed filename
    filename = "find_target_results.jpg"
    output_path = os.path.join(output_dir, filename)
    
    plt.savefig(output_path, dpi=300, bbox_inches='tight', format='jpg')
    plt.close()  # Close the figure to free memory

    print(f"Coordinate systems visualization results saved to: {output_path}")
    
    # Print transformation information
    print("\nSummarize coordinate system information...")
    print(f"Cabinet Frame origin in Base Frame (m): {cabinet_origin}")
    print(f"Target Frame origin in Base Frame (m): {task_pos_base}")
    print(f"Target Frame orientation (deg): {target_rpy_deg}")
    ...
    print(f"Cabinet Frame orientation relative to Base (deg): {[round(x, 2) for x in cabinet_rpy]}")
    print(f"Ideal Target Frame orientation relative to Base (deg): {[round(x, 2) for x in target_rpy_deg]}")
    
    # Show comparison between ideal and actual TCP target if available
    if actual_tcp_info is not None:
        actual_tcp_pos, actual_tcp_rpy, actual_tcp_rot = actual_tcp_info
        
        print("\nComparison between ideal and actual target frame:")
        
        # 1. Origin Position Comparison
        print("1. Origin Position:")
        print(f"   Ideal Target Frame origin (m):    {[round(float(x), 6) for x in task_pos_base]}")
        print(f"   Actual Target Frame origin (m):   {[round(float(x), 6) for x in actual_tcp_pos]}")
