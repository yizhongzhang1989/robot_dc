...
# ================================= Movement Functions ================================
    def movel_to_correct_tool_tcp(self):
        ...
        # Tool Y+ -> Crack Local Y- (negative Y)
        # Tool Z+ -> Crack Local Z- (negative Z)
        
        target_tool_rotation = np.column_stack([
            crack_local_x,     # Tool X+ = Crack Local X+
            -crack_local_y,    # Tool Y+ = Crack Local Y- (negative Y)
            -crack_local_z     # Tool Z+ = Crack Local Z- (negative Z)
        ])
        ...
        # The Z axis in tool corresponds to -crack_local_z in base coordinates
        rotation_axis = -crack_local_z / np.linalg.norm(crack_local_z)  # Normalize (should already be normalized)
    ...
    def movel_to_target_position(self):
        ...
        # Step 1: Move along crack local X and Y directions (keep crack local Z unchanged)
        # Calculate intermediate position by adding crack local X and Y movements
        intermediate_movement_x = movement_crack_local_x * crack_local_x
        intermediate_movement_y = movement_crack_local_y * crack_local_y
        ...
        print(f"\nStep 1: Moving along crack local X and Y directions...")
        ...
        if res != 0:
            print(f"Failed to move along crack local X and Y (error code: {res})")
            return res
        
        print("Step 1 completed successfully")

        
        # Step 2: Move along crack local Z direction to final target pulling position (between the two handles)
        tool_offset_z = 0.2265  # Tool offset along crack local Z direction (in meters)
        correct_offset = 0.10  # Correct offset to accurate (meters)
        final_movement_z = (movement_crack_local_z + tool_offset_z + correct_offset) * crack_local_z
        ...
        print(f"\nStep 2: Moving along crack local Z direction...")
        ...
        if res == 0:
            ...
            print(f"Failed to move along crack local Z (error code: {res})")
...
# ================================ Force Control Functions ================================
    def force_task_extract_server(self,distance):
        ...
        # This is the frame after Step 1 of movel_to_correct_tool_tcp (aligned with crack local coordinate system)
        
        # Extract rotation matrix from crack local coordinate system transformation matrix
        crack_local_rotation = self.crack_coord_transformation[:3, :3]
        crack_local_x = crack_local_rotation[:, 0]  # Crack Local X+ direction
        crack_local_y = crack_local_rotation[:, 1]  # Crack Local Y+ direction
        crack_local_z = crack_local_rotation[:, 2]  # Crack Local Z+ direction
    ...
    def force_task_touch_handle(self):
        """
        Execute force control task to touch handle based on crack local coordinate system.
        """
        if self.robot is None:
            print("Robot is not initialized")
            return -1
        ...
        # Set force mode parameters
        selection_vector = [1, 1, 1, 0, 0, 0]  # Enable force control in Z direction (now relative to task frame)
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    ur_handle1.force_task_touch_handle()
    time.sleep(0.5)
    ...
    print("\n" + "="*50)
    ur_handle1.movel_in_crack_frame([0,0.005,0.015])
    time.sleep(0.5)
    ...
    print("\n" + "="*50)
    ur_handle1.movel_in_crack_frame([0,0,0.20])
    time.sleep(0.5)
