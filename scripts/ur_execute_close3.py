...
class URExecuteClose3(URExecuteBase):
    def __init__(self, robot_ip="192.168.1.15", robot_port=30002):
        # Call parent class constructor first
        super().__init__(robot_ip, robot_port)
        
        # Override data_dir and result_dir with close3-specific paths
        self.data_dir = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "temp",
            "ur_locate_close3_data"
        )
        
        self.result_dir = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "temp",
            "ur_locate_close3_result"
        )
    ...
    def movel_to_right_handle_close3_position(self):
        """
        Move robot to right close3 position using linear movement based on local coordinate system.
        """
        if self.robot is None:
            print("Robot is not initialized")
            return -1
        ...
        print(f"\n[Right Close3] Movement vector in base coordinates: {movement_vector}")
        ...
        # Step 1: Move along local X and Z directions (keep local Y unchanged)
        # Calculate intermediate position by adding local X and Z movements
        # Apply -0.26m offset in local X direction for right close3, plus Z offset +0.01m
        ...
        print(f"\n[Right Close3] Step 1: Moving along local X and Z directions...")
        print(f"Intermediate pose: {intermediate_pose}")
        ...
        print(f"\n[Right Close3] Step 2: Moving along local Y direction...")
        print(f"Final pose: {final_pose}")
        ...
        if res == 0:
            print("Robot moved to right close3 position successfully")
        else:
            print(f"Failed to move along local Y (error code: {res})")
    ...
    def movel_to_correct_tool_tcp(self):
        ...
        # Local coordinate system axes in base close3
        ...
        # Construct target rotation matrix for tool close3:
        # Tool X+ -> Local X+
        ...
        # The Z axis in tool close3 corresponds to local_y in base close3
    ...
    def force_task_close3_right_handle(self):
        """
        Execute force control task to unlock the left close3 using TCP coordinate system.
        """
        if self.robot is None:
            print("Robot is not initialized")
            return -1
        ...
        # ==============Motion 1: push to close3 the handle=================
        print("\n[Motion 1] Pushing to close3 the handle...")
        ...
        # ==============Motion 3: push handle to completely close3=================
        print("\n[Motion 3] Pushing handle to completely close3...")
        ...
        print("[INFO] Close3 handle sequence completed successfully")
        return 0
...
if __name__ == "__main__":
    # Create URExecuteClose3 instance
    ur_close3 = URExecuteClose3()
    
    # All parameters are automatically loaded during initialization
    # Access them directly from instance variables
    
    if ur_close3.camera_matrix is not None:
        print("\nCamera Matrix:")
        print(ur_close3.camera_matrix)
        print("\nDistortion Coefficients:")
        print(ur_close3.distortion_coefficients)
    
    if ur_close3.cam2end_matrix is not None:
        print("\nCamera to End-effector Matrix:")
        print(ur_close3.cam2end_matrix)
    
    if ur_close3.estimated_keypoints is not None:
        print("\nEstimated Keypoints:")
        for kp in ur_close3.estimated_keypoints:
            print(f"Keypoint {kp['keypoint_index']}: ({kp['x']:.6f}, {kp['y']:.6f}, {kp['z']:.6f})")
    
    if ur_close3.local_transformation_matrix is not None:
        print("\nLocal Coordinate System Transformation Matrix:")
        print(ur_close3.local_transformation_matrix)
    
    if ur_close3.ref_joint_angles is not None:
        print("\nReference Joint Angles (radians):")
        for i, angle in enumerate(ur_close3.ref_joint_angles):
            print(f"Joint {i}: {angle:.6f}")
    
    if ur_close3.task_position_offset is not None:
        print("\nTask Position Offset (in local coordinate system):")
        print(f"x: {ur_close3.task_position_offset.get('x', 0):.6f}")
        print(f"y: {ur_close3.task_position_offset.get('y', 0):.6f}")
        print(f"z: {ur_close3.task_position_offset.get('z', 0):.6f}")

    # Calculate target position in base coordinate system
    target_position = ur_close3.get_target_position()
    ...
    print("\n" + "="*50)
    ur_close3.movej_to_reference_joint_positions()
    time.sleep(0.5)
    
    # align tool TCP with local coordinate system
    print("\n" + "="*50)
    ur_close3.movel_to_correct_tool_tcp()
    time.sleep(0.5)
    
    # move to target position using linear movement
    print("\n" + "="*50)
    ur_close3.movel_to_target_position()
    time.sleep(0.5)

    # move to touch left handle
    print("\n" + "="*50)
    ur_close3.force_task_touch_right_handle()
    time.sleep(0.5)

    # execute close3 left handle
    print("\n" + "="*50)
    ur_close3.force_task_close3_right_handle()
    time.sleep(0.5)

    # # move to leave the handle
    print("\n" + "="*50)
    ur_close3.movel_to_leave_handle([0, -0.2, 0])
    time.sleep(0.5)
