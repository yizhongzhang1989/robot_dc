...
class CameraViewerNode(Node):
    """ROS2 node for camera viewer with web interface."""
    
    def __init__(self, camera_topic='/ur15_camera/image_raw', web_port=8030):
    ...
    def _init_ur15_connection(self):
        ...
        try:
            ...
            if result == 0:
                self.get_logger().info(f"Connected to UR15 robot at {ur15_ip}:{ur15_port} for freedrive control")
            else:
        ...
        try:
            with self.robot_data_lock:
                ...
                if len(msg.position) >= 6:
                    # Rearrange: take last element (J1) and put it first
    ...
    def tcp_pose_callback(self, msg):
        ...
        try:
            with self.robot_data_lock:
                # Extract position (convert to mm) and orientation (convert to degrees if needed)
                pose = msg.pose
                self.tcp_pose = {
...
<head>
    ...
    <title>UR15 Camera Viewer</title>
    <style>
        body {
...
<body>
    <div class="container">
        <div class="header">
            <h1>📷 UR15 Camera Viewer</h1>
            <p>Real-time Camera View</p>
        </div>
    ...
    <script>
        ...
        function updateStatus() {
            ...
            .then(data => {
                ...
                console.log('Joint positions:', data.joint_positions, 'Length:', data.joint_positions ? data.joint_positions.length : 0);
                ...
                console.log('TCP pose:', data.tcp_pose);
                if (data.tcp_pose) {
    ...
    def project_base_origin_to_image(self, frame):
        ...
        try:
            # Check if we have all necessary calibration data
            with self.calibration_lock:
            ...
            with self.robot_data_lock:
                if self.tcp_pose is None:
                    return frame
                # Build end2base matrix from TCP pose
                # TCP pose is in mm and quaternion
            ...
            # Calculate cam2base: cam2base = end2base * cam2end (left multiply)
            cam2base_matrix = end2base_matrix @ cam2end_matrix
            
            # Base origin in base coordinates
            base_origin_base = np.array([0.0, 0.0, 0.0, 1.0])
            
            # Transform base origin to camera coordinates
            ...
            if point_3d[2] <= 0:
                # Point is behind camera, don't draw
                return frame
            ...
            try:
                with self.image_lock:
                    if self.current_image is not None:
                        ...
                        # Project and draw base origin on frame
...
def main(args=None):
    ...
    try:
        ...
        node = CameraViewerNode(camera_topic=camera_topic, web_port=web_port)
