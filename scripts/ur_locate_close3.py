#!/usr/bin/env python3
"""
UR Locate Close3 Script
This script inherits from URLocateBase and customizes it for close3 location tasks.
"""
...
class URLocateClose3(URLocateBase):
    def __init__(self, api_url="http://10.172.100.34:8001", robot_ip="192.168.1.15", robot_port=30002):
        """
        Initialize URLocateClose3 class for UR robot close3 location tasks
        ...
        self.get_logger().info('URLocateClose3 initialized')
        ...
        self.data_dir = os.path.join(self.script_dir, '..', 'temp', 'ur_locate_close3_data')
        
        # Override result directory path
        self.result_dir = os.path.join(self.script_dir, '..', 'temp', 'ur_locate_close3_result')
        ...
        # For close3 location, use keypoint 0 to keypoint 1 to define X-axis
        ...
        print(f"URLocateClose3 initialized with custom settings:")
...
def main():
    """
    Main function for URLocateClose3
    """
    # Initialize ROS2
    rclpy.init()
    
    try:
        # Initialize URLocateClose3 instance (robot connection is handled internally)
        ur_close3 = URLocateClose3()
        
        # Check if robot was initialized successfully
        if ur_close3.robot is None or not ur_close3.robot.connected:
            print("✗ Robot initialization failed. Please check robot connection and try again.")
            return
        
        # Use multi-threaded executor to handle callbacks
        executor = MultiThreadedExecutor()
        executor.add_node(ur_close3)
        ...
        try:
            ...
            if ur_close3.auto_collect_data():
                ...
                if ur_close3.estimate_3d_position():
                    ...
                    if coord_system:
                        ...
                        print("="*60)
                        if ur_close3.validate_local_coordinate_system(coord_system):
            ...
            if ur_close3.robot is not None:
                try:
                    ur_close3.robot.close()
