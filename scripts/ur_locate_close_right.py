#!/usr/bin/env python3
"""
UR Locate CloseRight Script
This script inherits from URLocateBase and customizes it for close_right location tasks.
"""
...
class URLocateCloseRight(URLocateBase):
    def __init__(self, ffpp_web_url="http://10.172.100.34:8001", robot_ip="192.168.1.15", robot_port=30002):
        ...
        # For close_right location, use keypoint 0 to keypoint 1 to define X-axis
...
def main():
    """
    Main function for URLocateCloseRight
    """
    # Initialize ROS2
    rclpy.init()
    
    try:
        # Initialize URLocateCloseRight instance (robot connection is handled internally)
        ur_close_right = URLocateCloseRight()
        
        # Check if robot was initialized successfully
        if ur_close_right.robot is None or not ur_close_right.robot.connected:
            print("✗ Robot initialization failed. Please check robot connection and try again.")
            return
        
        # Use multi-threaded executor to handle callbacks
        executor = MultiThreadedExecutor()
        executor.add_node(ur_close_right)
        ...
        try:
            ...
            if ur_close_right.auto_collect_data():
                ...
                if ur_close_right.estimate_3d_position():
                    ...
                    print("="*60)
                    if ur_close_right.validate_keypoints_3d_estimate_result():
                        print("✅ 3D estimation validation completed!")
                    else:
                        print("⚠ 3D estimation validation failed!")
                    
                    # Build keypoint coordinate system
                    coord_system = ur_close_right.build_local_coordinate_system()
                    if coord_system:
                        ...
                        print("="*60)
                        if ur_close_right.validate_local_coordinate_system(coord_system):
            ...
            if ur_close_right.robot is not None:
                try:
                    ur_close_right.robot.close()
